...
**TL;DR for the next agent (2026-08-22 — Prisma owns DB):**
...
4. Workflow: `edit prisma/schema.prisma` → `npx prisma migrate dev --name <feature>` → add raw SQL for triggers/RLS if needed → `npx prisma generate` → optionally `Copy-Item .../migration.sql ../../db/schema.sql -Force`. For storage: `npx supabase migration new add_<feature>` → edit only storage objects → `npx supabase db reset`.
5. Verification: `npx supabase start` must succeed with no FK violation (DB empty until `prisma migrate deploy`), `npx prisma migrate status` / `deploy` creates tables, `npx prisma db seed` is idempotent. See `AGENTS.md:6` checklist.
