...
| `FEE_*`, `MAX_BASKET_SPREAD_M` | Quote maths. **Decided** (issue #34): R18 base, R6 per extra shop, R3.50/km, courier take 75%, basket spread 1.5 km. Defaults live in `src/config/configuration.ts`. |
...
password. `admin@` is the only one that can call `/v1/admin/users/{id}/role`,
which is how someone becomes a shop owner or a courier.
